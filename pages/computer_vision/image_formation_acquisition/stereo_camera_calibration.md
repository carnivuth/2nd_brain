...
##  Why not calibrate both cameras with [zhang](computer_vision/image_formation_acquisition/zhang_method.md)

In order to calibrate a stereo camera system the $PPM$ alone is not sufficient, the rigid motion between cameras is also needed, a first approach could be to compute the [zhang's method](computer_vision/image_formation_acquisition/zhang_method.md) for both cameras but this approach as one major flaw, is not robust to noise
...
The first guess is obtained as the median between the $R_{i}$ $T_{i}$ computed by chaining the [transformations](computer_vision/image_formation_acquisition/perspective_projection_matrix.md#accounting%20for%20rotation%20and%20translation) $g_i(g_j^{-1})$
...
This is done by virtually rotating the calibrated cameras (e.g. redefining the $PPMs$ ) about their optical center through an [homography](computer_vision/image_formation_acquisition/homography.md)
...
Both images go trough a rotation and a change of intrinsic parameter, so they are related to the originals through [homographies](computer_vision/image_formation_acquisition/homography.md)
...
with a calibrated stereo system the [depth information](computer_vision/image_formation_acquisition/stereo_image_acquisition.md) and also 3D coordinates can be estimated, so given the relation between 3D point and image points
